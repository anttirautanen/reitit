import { use } from "react"
import { RouteContext } from "../RouteContext"
import { StaleIndicator } from "./StaleIndicator"

interface TopBarProps {
  onAddStop?: () => void
}

export const TopBar = ({ onAddStop }: TopBarProps) => {
  const { selectedRoute } = use(RouteContext)

  return (
    <div className="fixed top-0 left-0 right-0 z-10 h-11 bg-white border-b border-gray-200 flex items-center justify-between px-3">
      <button
        type="button"
        onClick={onAddStop}
        className="text-sm font-medium text-orange-600 hover:text-orange-500 active:text-orange-700 cursor-pointer focus:outline-2 focus:outline-offset-1 outline-black rounded px-1"
      >
        + Add stop
      </button>
      <h1 className="text-base font-medium truncate px-2">{selectedRoute.name}</h1>
      <div className="flex items-center gap-2">
        <StaleIndicator />
        <button
          type="button"
          aria-label="Settings"
          className="text-gray-600 hover:text-gray-800 cursor-pointer focus:outline-2 focus:outline-offset-1 outline-black rounded p-1"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="w-5 h-5"
            aria-hidden="true"
          >
            <circle cx="12" cy="12" r="3" />
            <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" />
          </svg>
        </button>
      </div>
    </div>
  )
}
